...
class KafkaApis(val requestChannel: RequestChannel,
    ...
    val response =
      if (authorize(request.session, ClusterAction, Resource.ClusterResource)) {
        ...
        result.foreach { case (topicPartition, errorCode) =>
          if (errorCode == Errors.NONE && stopReplicaRequest.deletePartitions() && topicPartition.topic == GroupMetadataTopicName) {
  ...
  def handleInitPidRequest(request: RequestChannel.Request): Unit = {
    ...
    def sendResponseCallback(result: InitPidResult): Unit = {
      ...
      trace(s"Completed $transactionalId's InitPidRequest with result $result from client ${request.header.clientId}")
      requestChannel.sendResponse(new RequestChannel.Response(request, responseBody))
    }
