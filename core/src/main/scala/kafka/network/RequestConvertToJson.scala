...
object RequestConvertToJson {
  def request(request: AbstractRequest): JsonNode = {
    request match {
      ...
      case req: ReadShareGroupOffsetsStateRequest => ReadShareGroupOffsetsStateRequestDataJsonConverter.write(req.data, request.version)
  ...
  def response(response: AbstractResponse, version: Short): JsonNode = {
    response match {
      ...
      case res: ReadShareGroupOffsetsStateResponse => ReadShareGroupOffsetsStateResponseDataJsonConverter.write(res.data, version)
