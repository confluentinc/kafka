...
public class MockProducer<K, V> implements Producer<K, V> {
    ...
    public synchronized Future<RecordMetadata> send(ProducerRecord<K, V> record, Callback callback) {
        ...
        FutureRecordMetadata future = new FutureRecordMetadata(result, 0, LogEntry.NO_TIMESTAMP, 0, 0, 0);
        long offset = nextOffset(topicPartition);
        Completion completion = new Completion(offset,
                                               new RecordMetadata(topicPartition, 0, offset, LogEntry.NO_TIMESTAMP, 0, 0, 0),
                                               result, callback);
    ...
    private static class Completion {
        ...
        public void complete(RuntimeException e) {
            result.set(e == null ? offset : -1L, LogEntry.NO_TIMESTAMP, e);
            if (callback != null) {
                if (e == null)
