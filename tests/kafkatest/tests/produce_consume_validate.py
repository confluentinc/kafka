...
class ProduceConsumeValidateTest(Test):
    ...
    def start_producer_and_consumer(self, async=False):
        ...
        self.producer.start()
        wait_until(lambda: self.producer.num_acked > 5 or async, timeout_sec=20,
             err_msg="Producer failed to start in a reasonable amount of time.")
        self.consumer.start()
        wait_until(lambda: len(self.consumer.messages_consumed[1]) > 0 or async, timeout_sec=60,
    ...
    def validate(self):
        ...
        missing = set(map(int, acked)) - set(map(int, consumed))
