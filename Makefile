...
compile-validate:
	./retry_zinc ./gradlewAll clean install build -x test --no-daemon --stacktrace -PxmlSpotBugsReport=true 2>&1 | tee -a build.log
...
check-scala-compatibility:
	./retry_zinc ./gradlew clean build -x test --no-daemon --stacktrace -PxmlSpotBugsReport=true -PscalaVersion=2.12 2>&1 | tee -a build.log
	@error_count=$$(grep -c -E "(ERROR|error:|\[Error\]|FAILED)" build.log); \
	if [ $$error_count -ne 0 ]; then \
		grep -E "(ERROR|error:|\[Error\]|FAILED)" build.log | while read -r line; do \
			echo "$$line"; \
